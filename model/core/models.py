...
import torch.nn as nn
import torch.nn.parameter
from core.attentionLayer import attentionLayer
...
#         self.sat = SelfAttentionModule((512), hidden_units)
#         self.lstm = nn.LSTM((512), hidden_units, batch_first=True)
...
class ASC_Net(nn.Module):
    def __init__(self, clip_number=11, candidate_speakers=3, hidden_units=128):
        ...
        self.facePose = nn.Linear(512+39, 512)
    ...
    def forward(self, x):
        x = x.permute(3, 0, 2, 1)  # B * C * T* S (channel) => S*B*T*C     
        tempX = []
        for i in range(x.shape[0]):
            # create an additional linear layer
            x1=x[i,:,:,:512]
            x2=self.facePose(x[i,:,:,512:])
            x1_c = self.crossA2V(src = x1, tar = x2)
            x2_c = self.crossV2A(src = x2, tar = x1)
            tempX.append(torch.cat((x1_c,x2_c), 2))
        x=torch.stack(tempX)
        x  = x.permute(1, 3, 2, 0)# S*B*T*C  (channel) => B * C * T* S 
        
        
        # Pairwise Attention
        x = self.sat(x)

        # Temporal Module with LSTM 1
        x = x.reshape(x.size(0), x.size(1), -1)
        x = x.permute(0, 2, 1)
        x, _ = self.lstm(x)
        ...
        x = x.permute(0, 2, 1) # Reshape back to (batch_size, sequence_length, hidden_units)
        
        x = x.permute(0, 2, 1) # Reshape back to (batch_size, hidden_units, sequence_length)
        # x = self.norm1(x)
