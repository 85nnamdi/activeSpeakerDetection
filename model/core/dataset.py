...
class ContextualDataset(data.Dataset):
    ...
    def _decode_feature_data_from_csv(self, feature_data):
        #############################################################################
        ## See a new change to the indexing from -1 to 39 to get ride of the pose vector
        #############################################################################
        feature_data = feature_data[1:-1]
        feature_data = feature_data.split(',')
        # Changed on this line return np.asarray([float(fd) for fd in feature_data][:-39])
        
        # audio = np.asarray([float(fd) for fd in feature_data][:512])
        # visual = np.asarray([float(fd) for fd in feature_data][512:-39])
        # pose = np.asarray([float(fd) for fd in feature_data][-39:])
...
class ASCFeaturesDataset(ContextualDataset):
    ...
    def __getitem__(self, index):
        ...
        feature_set = np.zeros((self.candidate_speakers, self.time_lenght, (1063)))#1063
